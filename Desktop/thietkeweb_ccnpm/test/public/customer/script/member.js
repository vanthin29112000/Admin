...
function loginCustomer() {
    formDataLogin.addEventListener('submit', (e) => {
        ...
        if (checkSpace(formDataLogin['login-username'].value)) document.getElementById('login-warning-username').style.opacity = '1';
        else document.getElementById('login-warning-username').style.opacity = '0';
        if (checkSpace(formDataLogin['login-password'].value)) document.getElementById('login-warning-password').style.opacity = '1';
        else document.getElementById('login-warning-password').style.opacity = '0';
        ...
        // Login user[Member]
        firebase.firestore().collection("member").doc(formDataLogin['login-username'].value).get().then((doc) => {
            if (doc.exists) {
                firebase.auth().signInWithEmailAndPassword(doc.data().email, formDataLogin['login-password'].value)
                .then(() => {
                    // notify.play();
                    load_login();
...
function signUpCustomer() {
    formDataSignUp.addEventListener('submit', (e) => {
        ...
        if (formDataSignUp['signup-fullname'].value == '') document.getElementById('signup-warning-fullname').style.opacity = '1';
        else document.getElementById('signup-warning-fullname').style.opacity = '0';
        
        if (formDataSignUp['signup-birthdate'].value == '') document.getElementById('signup-warning-birthdate').style.opacity = '1';
        else document.getElementById('signup-warning-birthdate').style.opacity = '0';

        if (formDataSignUp['gender'].value == '') document.getElementById('signup-warning-gender').style.opacity = '1';
        else document.getElementById('signup-warning-gender').style.opacity = '0';

        if (formDataSignUp['signup-tel'].value == '') document.getElementById('signup-warning-tel').style.opacity = '1';
        else {
        ...
        if (checkSpace(formDataSignUp['signup-username'].value)) document.getElementById('signup-warning-username').style.opacity = '1';
        else document.getElementById('signup-warning-username').style.opacity = '0';

        if (checkSpace(formDataSignUp['signup-email'].value)) document.getElementById('signup-warning-email').style.opacity = '1';
        else document.getElementById('signup-warning-email').style.opacity = '0';

        if (checkSpace(formDataSignUp['signup-password'].value)) document.getElementById('signup-warning-password').style.opacity = '1';
        else document.getElementById('signup-warning-password').style.opacity = '0';

        if (formDataSignUp['signup-repassword'].value != formDataSignUp['signup-password'].value)
            document.getElementById('signup-warning-repassword').style.opacity = '1';
        else document.getElementById('signup-warning-repassword').style.opacity = '0';

        if (formDataSignUp['signup-address'].value == '') document.getElementById('signup-warning-address').style.opacity = '1';
        else document.getElementById('signup-warning-address').style.opacity = '0';
        ...
        if (
            ...
            (formDataSignUp['signup-tel'].value != '' && isNum(formDataSignUp['signup-tel'].value)) &&
            ...
            formDataSignUp['signup-repassword'].value == formDataSignUp['signup-password'].value &&
            formDataSignUp['signup-address'].value != ''
        ) 
        {
            db.collection("member").doc(formDataSignUp['signup-username'].value).get().then((doc)=>{
                if(doc.exists)
                {
                    ...
                    firebase.auth().createUserWithEmailAndPassword(formDataSignUp['signup-email'].value.trim(), formDataSignUp['signup-password'].value.trim()) // formDataSignUp['signup-password'].value.trim()
                    .then(() => {
                        firebase.auth().currentUser.updateProfile({
                            displayName: formDataSignUp['signup-username'].value.trim()
                        });
                        firebase.firestore().collection("member").doc(formDataSignUp['signup-username'].value).set({
                            fullname: formDataSignUp['signup-fullname'].value,
                            birthdate: firebase.firestore.Timestamp.fromDate(new Date(formDataSignUp['signup-birthdate'].value)),
                            ...
                            notify.play();
                            notification('Đăng ký tài khoản thành công!',true);
                            auth.signOut();
                            ...
                            gotoSignUp.style.boxShadow = 'inset 20px 20px 60px rgb(190, 190, 190), inset -20px -20px 60px rgb(255, 255, 255)';
...
function update_info_member(){
        ...
        // Check data input of sign up
        if (fromDataChangeInfo['change-fullname'].value == '') document.getElementById('change-warning-fullname').style.opacity = '1';
        else document.getElementById('change-warning-fullname').style.opacity = '0';
        
        if (fromDataChangeInfo['change-birthdate'].value == '') document.getElementById('change-warning-birthdate').style.opacity = '1';
        else document.getElementById('change-warning-birthdate').style.opacity = '0';

        if (fromDataChangeInfo['gender'].value == '') document.getElementById('change-warning-gender').style.opacity = '1';
        else document.getElementById('change-warning-gender').style.opacity = '0';

        if (fromDataChangeInfo['change-tel'].value == '') document.getElementById('signup-warning-tel').style.opacity = '1';
        else {
        ...
        if (checkSpace(fromDataChangeInfo['change-username'].value)) document.getElementById('change-warning-username').style.opacity = '1';
        else document.getElementById('change-warning-username').style.opacity = '0';

        if (checkSpace(fromDataChangeInfo['change-email'].value)) document.getElementById('change-warning-email').style.opacity = '1';
        else document.getElementById('change-warning-email').style.opacity = '0';

        if (checkSpace(fromDataChangeInfo['change-password'].value)) document.getElementById('change-warning-password').style.opacity = '1';
        else document.getElementById('change-warning-password').style.opacity = '0';

        if (fromDataChangeInfo['change-repassword'].value != fromDataChangeInfo['change-password'].value)
            document.getElementById('change-warning-repassword').style.opacity = '1';
        else document.getElementById('change-warning-repassword').style.opacity = '0';

        if (fromDataChangeInfo['change-address'].value == '') document.getElementById('change-warning-address').style.opacity = '1';
        else document.getElementById('change-warning-address').style.opacity = '0';

        if (
            fromDataChangeInfo['change-fullname'].value != '' &&
            fromDataChangeInfo['change-birthdate'].value != '' &&
            fromDataChangeInfo['gender'].value != '' &&
            (fromDataChangeInfo['change-tel'].value != '' && isNum(fromDataChangeInfo['change-tel'].value)) &&
        ...
        ) 
        {
            let flag = true;
            // update profile
            db.collection("member").doc(auth.currentUser.displayName).update({
                fullname : updateDataProfile['change-fullname'].value,
                gender : updateDataProfile['gender'].value,
                birthdate : firebase.firestore.Timestamp.fromDate(new Date(updateDataProfile['change-birthdate'].value)),
...
function login_Again(){
    fromLoginAgain.addEventListener('submit', (e) => {
        e.preventDefault();
        // Check data input of login
        if (checkSpace(fromLoginAgain['login-username-again'].value)) document.getElementById('login-warning-username-again').style.opacity = '1';
        else document.getElementById('login-warning-username-again').style.opacity = '0';
        if (checkSpace(fromLoginAgain['login-password-again'].value)) document.getElementById('login-warning-password-again').style.opacity = '1';
        else document.getElementById('login-warning-password-again').style.opacity = '0';
        ...
        auth.currentUser.reauthenticateWithCredential(credential).then(function() {
            auth.currentUser.updatePassword(fromDataChangeInfo['change-password'].value).then(function(){
                notification("Thay đổi mật khẩu thành công ",true); 
                close_Login_Again();
            }).catch(function(e){
                // notification(e.message,false);
                document.getElementById('login-warning-password-again').style.opacity = '1';
            })
        }).catch((error) => {
            // notification(error,false);
            document.getElementById('login-warning-password-again').style.opacity = '1'
...
var interval_obj = setInterval(()=>{
    ...
    document.getElementById('nameCurrentUser').innerHTML = firebase.auth().currentUser.displayName;
    document.getElementsByClassName("c-user-1")[0].innerHTML += 
        `<div class="c-currentUser">
...
function close_update_profile(){
    document.getElementsByClassName("change-info-member")[0].style.display ="none";
}

function update_profile()
{

    db.collection("member").doc(auth.currentUser.displayName).get().then((doc)=>{
        if(doc.exists){
            ...
            let temp_string = temp_date.getFullYear() + "-" +("0"+(temp_date.getMonth() +1)).substr(-2) + "-" + ("0"+ temp_date.getDate()).substr(-2) ;
    ...
    document.getElementsByClassName("change-info-member")[0].style.display ="block";
    document.getElementById("change-password").value = "";
    document.getElementById("change-repassword").value = "";
}

function reset_password(){ //change from email to username
    ...
    document.querySelector('.reset-pass').addEventListener('submit',(e)=>{
        ...
        db.collection("member").doc(username).get().then((user)=>{
            if(user.exists)
            {
                ...
                auth.sendPasswordResetEmail(user.data().email).then(function() {
                    alert("Kiểm tra tài khoản Email "+user.data().email+" để reset lại mật khẩu !!!");
                }).catch((error) =>{
                    console.log(error.message);
                });
            }
...
cart.addEventListener('click', () => {console.log('Go to Gio Hang!')});
...
gotoSignUp.addEventListener('click', () => {
    ...
    gotoLogin.style.boxShadow = 'inset 20px 20px 60px rgb(190, 190, 190), inset -20px -20px 60px rgb(255, 255, 255)';
...
document.getElementsByClassName("plus")[0].addEventListener("mouseover",()=>{
    document.getElementsByClassName("drop-down-icon-a")[0].style.display = "block";
});

document.getElementsByClassName("plus")[0].addEventListener("mouseout",()=>{
    document.getElementsByClassName("drop-down-icon-a")[0].style.display = "none";
});